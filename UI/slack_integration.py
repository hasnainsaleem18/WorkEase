...
class SlackIntegration:
    """Slack API Integration for WorkEase"""
    ...
    def get_conversations(self, types='public_channel,private_channel,mpim,im', exclude_archived=True):
    ...
    def parse_message(self, message):
        ...
        if parsed['user']:
            ...
            if user_info:
                parsed['user_name'] = user_info.get('real_name', user_info.get('name', ''))
                parsed['user_display_name'] = user_info.get('profile', {}).get('display_name', '')
        ...
        if auth_result['success']:
            try:
                ...
                return {
                    ...
                    'channels_count': len([c for c in convos if not c.get('is_im', False)]),
