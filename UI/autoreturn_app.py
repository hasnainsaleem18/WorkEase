...
class AutoReturnApp(QMainWindow):
    def __init__(self):
        ...
        self.ollama_service = OllamaService(model_name="gpt-oss:120b-cloud")  # Using cloud model for better performance
    ...
    def create_main_content(self):
        ...
        filter_buttons = [
            ("All", "all", None),
            ("Gmail", "gmail", os.path.join(os.path.dirname(__file__), "Gmail_Logo_32px.png")),
            ("Slack", "slack", os.path.join(os.path.dirname(__file__), "icons8-slack-new-48.png")),
            ("Urgent", "urgent", os.path.join(os.path.dirname(__file__), "notification-bell-red.png"))
        ]
    ...
    def populate_table(self):
        ...
        for row, msg in enumerate(filtered):
            ...
            if msg.get('source') == 'gmail':
                pixmap = QPixmap(os.path.join(os.path.dirname(__file__), "Gmail_Logo_32px.png"))
            else:
                pixmap = QPixmap(os.path.join(os.path.dirname(__file__), "icons8-slack-new-48.png"))
    ...
    def on_table_cell_clicked(self, row, column):
        ...
        # Column 4 is "AI Summary"
        if column == 4:
            # Find the message corresponding to this row
            # Since we can't easily map row to message due to sorting/filtering,
            # we'll iterate through visible messages or use the widget text to find it
            
            # Better approach: Get the summary text from widget to identify the message
            # But wait, multiple messages might have same summary.
            # Let's try to find the message by matching the summary text
            
            widget = self.table.cellWidget(row, column)
            if isinstance(widget, QLabel):
                summary_text = widget.text()
                
                # Find the full analysis for this message
                full_text = summary_text
                for msg in self.messages:
                    if msg.get('summary') == summary_text:
                        full_text = msg.get('ai_analysis', summary_text)
                        break
