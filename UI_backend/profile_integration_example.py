...
class WorkEaseApp(QMainWindow):
    ...
    def set_user_info(self, user_data):
        ...
        if "connected_accounts" not in self.user_data:
            self.user_data["connected_accounts"] = {"gmail": False, "slack": False}
    ...
    def show_profile_dialog(self):
        ...
        if not self.user_data:
            QMessageBox.warning(
                self, "Not Logged In", "Please log in first to view your profile."
            )
            return
...
def save_user_data(user_data):
    ...
    keyring.set_password("workease", "user_data", json.dumps(user_data))
